...
public class BookChatController {
    ...
    Map<String,Object> send(Authentication auth,@PathVariable UUID bookId,@PathVariable UUID threadId,
        ...
        if(!existing.isEmpty()) {
            ...
            if(!input.mode().equals(previous.get("mode")) || !input.question().strip().equals(previous.get("question")))
        ...
        if("grounded".equals(input.mode())) {
            ...
            if(confirmed<=0) throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Сначала сохраните позицию чтения или выберите режим по знаниям модели");
            ...
            source=BookChatContext.select(original,confirmed,question,sections);
        ...
        var history=jdbc.queryForList("SELECT question,answer FROM book_chat_turns WHERE thread_id=? AND status='ready' AND mode=? ORDER BY created_at DESC,id DESC LIMIT 3",threadId,input.mode());
        Collections.reverse(history);
        var messages=new ArrayList<BookChatClient.Message>();
        String title=(String)book.get("title"),author=(String)book.get("author");
        if(source!=null) {
            messages.add(new BookChatClient.Message("system",BookChatGuard.SYSTEM_BOUNDARY+"Для фактов используй только приведённые отрывки загруженной книги. Предыдущие реплики служат только для понимания вопроса, не как доказательство. Ссылайся на номера отрывков [1], [2] и не придумывай ссылки. Если отрывков недостаточно, прямо скажи об этом. Не утверждай, что знаешь неприсланные части книги. Отвечай по-русски, кратко и по существу."));
        } else {
        ...
        int inserted=jdbc.update("INSERT INTO book_chat_turns(id,thread_id,mode,model_name,question,status,position_offset,source_sha256,source_updated_at,source_ranges,source_excerpt) VALUES (?,?,?,?,?,'pending',?,?,?,?::jsonb,?) ON CONFLICT DO NOTHING",
                input.id(),threadId,input.mode(),settings.model(),question,source==null?null:source.positionOffset(),
