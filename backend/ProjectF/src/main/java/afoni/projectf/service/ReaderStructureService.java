...
public class ReaderStructureService {
    ...
    public void save(UUID documentId,BookTextExtractor.ExtractedBook book) {
        ...
        jdbc.update("UPDATE documents SET author=? WHERE id=?",clean(book.author(),500),documentId);
        ...
        jdbc.update("UPDATE documents SET reader_structure_version=3 WHERE id=?",documentId);
    }
    @Transactional
    public synchronized void ensure(UUID documentId,String format) {
        ...
        int requiredVersion="epub".equals(format)?3:2;
    ...
    public void refreshOutdatedEpubStructures() {
        var ids=jdbc.query("SELECT id FROM documents WHERE source_type='epub' AND reader_structure_version<3 ORDER BY created_at",
